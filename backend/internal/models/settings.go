...
type Settings struct {
	...
	BuildProvider                SettingVariable `key:"buildProvider,envOverride" meta:"label=Build Provider;type=select;keywords=build,buildkit,depot,provider,remote,local;category=timeouts;description=Default build provider (local or depot)"`
	BuildkitEndpoint             SettingVariable `key:"buildkitEndpoint,envOverride" meta:"label=BuildKit Endpoint;type=text;keywords=buildkit,endpoint,daemon,host,unix,tcp;category=timeouts;description=BuildKit daemon address (e.g., unix:///run/buildkit/buildkitd.sock)"`
	BuildTimeout                 SettingVariable `key:"buildTimeout,envOverride" meta:"label=Build Timeout;type=number;keywords=build,timeout,seconds,buildkit;category=timeouts;description=Timeout for BuildKit builds in seconds (default: 1800 = 30 minutes)"`
	DepotProjectId               SettingVariable `key:"depotProjectId,envOverride" meta:"label=Depot Project ID;type=text;keywords=depot,project,id,build,provider;category=timeouts;description=Depot project identifier"`
	DepotToken                   SettingVariable `key:"depotToken,envOverride,sensitive" meta:"label=Depot Token;type=password;keywords=depot,token,api,secret,build,provider;category=timeouts;description=Depot API token"`
